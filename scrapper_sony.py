...
#    print(output)
    ...
    while (1):
        ...
        try:
            ...
            if tt:
                ...
                indexesEventBlocked = [[m.start(), m.end(), 1] for m in re.finditer('<tr class=\"table__row _state_blocked"(.)*?</tr>', sout)]
                indexes = sorted(indexesSegment + indexesEvent + indexesEventBlocked, key=lambda x: x[0])
                #print(indexes)
    ...
    #                        ss = re.findall('segment[0-9]*', sout[ind[0]:ind[1]])
                            segmentId = sout[ind[0]:ind[1]].split('<span class="table__title-text"')[1].split('>')[1].split('<')[0]
                            print(segmentId)
                    if flLine == 1:
                        if segmentId != '':
    ...
    #                print(e)
                ...
                driver.find_element_by_xpath('//div[@class="events__filter _type_sport _state_expanded"]/*/span[@class="events__filter-text"]').click()
